...
typedef enum {
    OP_CURLDIV=0,OP_SURFAREA=1,OP_SURFINT=2,OP_FLUX=3,
    OP_VOLUME=4,OP_TRIPLE=5,OP_TANGPLANE=6,OP_COUNT=7
} Op;
typedef enum { COORD_CART=0,COORD_CYL=1,COORD_SPH=2 } Coords;
typedef enum { SCR_MENU=0,SCR_INPUT=1,SCR_RESULT=2,SCR_ORIENT=3,SCR_COORDS=4 } Screen;

typedef struct { char label[14]; char val[MAX_EXPR]; char def[MAX_EXPR]; } Field;
typedef struct { char label[22]; char value[52]; } ResLine;
typedef struct {
    Screen screen; Op op; int menu_cur; int field_cur;
    int orient; Coords coords;
    Field f[MAX_FIELDS]; int nf;
    ResLine res[MAX_RES]; int nres;
    char err[64]; double raw;
} State;
...
static inline double cross_mag(double ax,double ay,double az,double bx,double by,double bz){
    double cx=ay*bz-az*by,cy=az*bx-ax*bz,cz=ax*by-ay*bx;
    return sqrt(cx*cx+cy*cy+cz*cz);
}
static inline void cross3(double ax,double ay,double az,double bx,double by,double bz,
                           double*cx,double*cy,double*cz){
    *cx=ay*bz-az*by;*cy=az*bx-ax*bz;*cz=ax*by-ay*bx;
}
static inline void fmt(char*buf,int n,double v){
    double r=round(v*1e6)/1e6; long long ri=(long long)r;
